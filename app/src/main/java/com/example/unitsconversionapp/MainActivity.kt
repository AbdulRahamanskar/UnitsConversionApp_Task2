package com.example.unitsconversionapp
...
//    #,###.####
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        showAllButton.setOnClickListener {
            ...
            if (input != null) {
                val rates = conversionRates[category] ?: mapOf()
                val fromRate = rates[fromUnit] ?: 1.0
                val conversions = StringBuilder()

                // Loop through each unit in the category and compute the conversion
                for ((unit, toRate) in rates) {
                    if (unit != fromUnit) { // Skip conversion to the same unit
                        val result = input * (toRate / fromRate)
                        conversions.append("${decimalFormat.format(input)} $fromUnit = ${decimalFormat.format(result)} $unit\n")
                    }
                }

                resultTextView.text = conversions.toString()
            } else {
